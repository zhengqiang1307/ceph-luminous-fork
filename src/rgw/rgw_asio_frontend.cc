...
class StreamIO : public rgw::asio::ClientIO { // 实现了write_data，recv_body
  Stream& stream;
                       ...
                       boost::asio::yield_context yield)
{
  ...
  for (;;) {
    ...
    parser.header_limit(header_limit); // 限制头大小
    parser.body_limit(body_limit); // 限制尾大小
    ...
    if (ec) {
      ...
      auto& message = parser.get();
      beast::http::response<beast::http::empty_body> response; // 响应的内容为空
      response.result(beast::http::status::bad_request); // 响应的状态码，整数
      response.version(message.version() == 10 ? 10 : 11); // HTTP-version
      response.prepare_payload(); // 准备消息的数据体
      beast::http::async_write(stream, response, yield[ec]); // 向客户端发生response消息
      if (ec) {
    ...
    // process the request
    // 构造request对象
    RGWRequest req{env.store->get_new_req_id()}; // 分配一个新的request id，创建一个request

    // 实际调用stream是beast::ip::socket
    auto& socket = stream.lowest_layer(); // stream绑定的底层I/O对象socket
                            ...
                            rgw::io::add_buffering(cct,
                              rgw::io::add_chunking(
                                rgw::io::add_conlen_controlling(
                                  &real_client))));
    // RGWRestfulIO: high level interface to interact with RESTful clients
...
class AsioFrontend {
  ...
  boost::asio::io_service service; // link to OS
#ifdef WITH_RADOSGW_BEAST_OPENSSL
  boost::optional<ssl::context> ssl_context;
  int init_ssl();
#endif

  // 内部类，使用io_service创建acceptor，socket
  struct Listener {
...
int AsioFrontend::init()
{
  ...
  auto& config = conf->get_config_map(); // 返回的config类型是multimap<string,string>
  ...
  // parse endpoints
  // 可以配置多个port，从而在多个端口监听
  // 通常一个端口对应一个进程，也就是说可以启动多个rgw服务
  auto ports = config.equal_range("port"); // multimap有序，从config读key是port的迭代器范围
  for (auto i = ports.first; i != ports.second; ++i) { // 遍历
    auto port = parse_port(i->second.c_str(), ec); // 返回端口值，从char*到uint转换
    if (ec) { // ec非零错误处理
      lderr(ctx()) << "failed to parse port=" << i->second << dendl;
      return -ec.value();
    }
    listeners.emplace_back(service); // 用service构造listener
    listeners.back().endpoint.port(port); // 设置端口值
  }

  // 配置endpoint，或port均可
  auto endpoints = config.equal_range("endpoint"); // multimap有序，从config读key是endpoint的迭代器范围
  ...
  for (auto& l : listeners) {
    ...
    // The parameter given to Socket.Listen is not the maximum number of connections, 
    // rather it is the size of the "backlog" of incoming connections.
    l.acceptor.listen(boost::asio::socket_base::max_connections); // 128
    // 异步accept，接受client连接后调用accept
...
void AsioFrontend::accept(Listener& l, boost::system::error_code ec)
{
  ...
  tcp::no_delay options(l.use_nodelay); // 跟新nodelay
...
// 使用bind/lambda方式来包装，满足以上签名
    boost::asio::spawn(service, std::bind(
      [this] (boost::asio::yield_context yield, tcp::socket& s) mutable {
        beast::flat_buffer buffer;
        boost::system::error_code ec;
        handle_connection(env, s, buffer, false, ec, yield); // 处理连接
        s.shutdown(tcp::socket::shutdown_both, ec); // 处理完毕连接后关闭连接
      }, std::placeholders::_1, std::move(socket)));
  }
}
...
{
  auto cct = ctx(); // 上下文
  const int thread_count = cct->_conf->rgw_thread_pool_size; // 从上下文配置中拿到线程池大小
  threads.reserve(thread_count); // vector.reserve()常用于避免使用过多内存，精准分配
  ...
  for (int i = 0; i < thread_count; i++) {
    threads.emplace_back([=] {
      for (;;) { // 死循环
        // Multiple threads may call the run() function to set up 
        // a pool of threads from which the io_service may execute handlers.

        // or This makes it possible for the I/O service object to 
        // use those threads to execute handlers when asynchronous operations complete. 

        // The advantage of threads is lost if handlers can’t be executed concurrently.
        service.run(); // 线程共用一个service，成为工作线程
        if (going_down) { // 线程退出条件
          break;
        }
        pauser.wait();
      }
