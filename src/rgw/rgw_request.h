...
{
  uint64_t id; 
  struct req_state *s; // 保存所有必需的状态
  string req_str; // 请求字符串
  RGWOp *op; // 基类指针，RGWOp提供了所有的操作函数
  utime_t ts;
  ...
  void log_format(struct req_state *s, const char *fmt, ...); // 格式化打印日志
  void log_init(); // 初始化日志
  void log(struct req_state *s, const char *msg); // 打印日志
}; /* RGWRequest */
